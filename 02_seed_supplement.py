...
def train():
    ...
    save_dir = "Result/Tournament_10k_20260306_1700"  # 固定到原文件夹
    os.makedirs(save_dir, exist_ok=True)

    roster = {"DQN (LSTM)": LSTMDQNAgent}
    raw_evidence = {}
    final_report = []
    ...
    for name, AgentClass in roster.items():
        ...
        final_report.append({
            "Model": name,
            "Mean Score": tail_mean(raw_evidence[f"{name}_r"], 100),
            "EHR": tail_mean(raw_evidence[f"{name}_ehr"], 100),
            "HFR": tail_mean(raw_evidence[f"{name}_hfr"], 100),
            "PPR": tail_mean(raw_evidence[f"{name}_ppr"], 100),
